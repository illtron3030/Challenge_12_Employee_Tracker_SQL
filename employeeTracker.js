...
const viewAllEmployees = () => {
  connection.query(
    `SELECT employee.id, employee.first_name AS 'First Name', employee.last_name AS 'Last Name', role.title as Title, department.name AS Department, 
    CONCAT('$', format(role.salary,0)) AS Salary, CONCAT(manager.first_name, ' ', manager.last_name) AS Manager 
    FROM employee 
    JOIN role ON employee.role_id = role.id 
    JOIN department ON role.department_id = department.id 
    LEFT JOIN employee AS manager ON employee.manager_id = manager.id ORDER BY employee.id ASC;`,
    (err, res) => {
      console.table(res);
      questions();
    }
      ...
      departments.forEach((department) => {
        if (department.value === answer.departmentId) {
          let sql = `INSERT INTO role (title, salary, department_id) VALUES ('${
            answer.roleTitle
          }', ${parseInt(answer.roleSalary)}, ${parseInt(
            answer.departmentId
          )})`;
      ...
      inquirer
        ...
        .then((answer) => {
          ...
          let sql = `INSERT INTO employee (first_name, last_name, role_id, manager_id ) VALUES ('${
            answer.firstName
          }', '${answer.lastName}', ${parseInt(answer.role)}, ${parseInt(
            answer.manager
          )})`;
          connection.query(sql, (err, res) => {
            viewAllEmployees();
...
const updateEmployeeRole = () => {
  ...
  connection.query(`SELECT * FROM employee`, (err, res) => {
    ...
    connection.query(`SELECT * FROM role`, (err, res) => {
      ...
      //choose employee would you like to update
      inquirer
        .prompt([
          {
            type: "list",
            name: "employee",
            message:
              "choose an employee you would like to update thier role for?",
            choices: employees,
          },
          {
            type: "list",
